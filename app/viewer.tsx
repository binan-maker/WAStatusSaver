...
export default function ViewerScreen() {
  const params = useLocalSearchParams<{ uri: string; type: string; name: string; id: string; isSaved?: string }>();
  ...
  const handleSave = useCallback(async () => {
    ...
    const success = await saveStatus({
      id: id || '',
      uri: uri || '',
  ...
  }, [isSaved, isSaving, saveStatus, id, uri, type, name]);
  ...
  }, [isSavedView, savedItems, id, deleteFromSaved]);
  ...
  return (
    <View style={styles.root}>
      ...
      <Animated.View
        style={[styles.topBar, { paddingTop: insets.top + 8, opacity: controlsOpacity, pointerEvents: (showControls || type === 'image') ? 'auto' : 'none' }]}
      >
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn} hitSlop={{ top: 8, right: 8, bottom: 8, left: 8 }}>
          <Ionicons name="arrow-back" size={22} color="#fff" />
        </TouchableOpacity>
        <Text style={styles.topTitle} numberOfLines={1}>{name || 'Status'}</Text>
        <View style={{ width: 40 }} />
      </Animated.View>

      <Animated.View
        style={[styles.bottomBar, { paddingBottom: insets.bottom + 16, opacity: controlsOpacity, pointerEvents: (showControls || type === 'image') ? 'auto' : 'none' }]}
      >
        <TouchableOpacity

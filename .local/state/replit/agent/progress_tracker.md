...
[x] REPLIT MIGRATION FINAL: All npm packages installed, Start Backend (port 5000) running, Start Frontend (Expo Metro port 8081) running. Project successfully migrated to Replit environment.
[x] 92. **FIREBASE AUTH PERSISTENCE:** Fixed auth state persistence - now uses initializeAuth with getReactNativePersistence(AsyncStorage) so users stay logged in across app restarts. Falls back to getAuth if already initialized.
[x] 93. **LINEARGRADIENT NULL FIX:** Added missing COLORS.ACCENT_GOLD = '#FFB800' to constants/colors.ts. This was causing NullPointerException crash in SupportDeveloperAd and guide.tsx which used COLORS.ACCENT_GOLD in gradients and tag colors.
[x] 94. **EDGE-TO-EDGE WARNING FIX:** Removed NavigationBar.setBehaviorAsync('inset-touch') call from applyImmersiveMode in _layout.tsx. This API is not supported in edge-to-edge mode and was generating unnecessary warnings.
[x] 95. **COMPLETE PRIVACY POLICY REWRITE (LEGAL):** Full 18-section legally comprehensive policy covering: beta disclaimer, Google Sign-In data, internet services (Firebase/Razorpay/AdMob), subscription terms, 30-day refund policy with eligibility/exclusions, data retention, disclaimer of warranties, limitation of liability, WhatsApp disclaimer, governing law (India), GDPR compliance. No legal loopholes left.
[x] 96. **COMPLETE SETUP GUIDE REWRITE:** Updated guide with new sections for Google Sign-In (optional, how to use), Subscription & Ad-Free Access (plans, refund info), Referral Program (all tiers), Push Notifications setup, updated Privacy & Data Safety section (reflects internet usage), and Beta badge. All existing Android setup sections preserved.
[x] 97. **SIGN OUT BUTTON AT BOTTOM OF SETTINGS:** Added a prominent red "Sign Out" button at the very bottom of the Settings scroll view (before footer), only visible when signed in.
[x] 98. **DELETE ACCOUNT WITH 30-DAY RETENTION:** Added "Delete Account" button inside the Account section. Double-confirmation alert explains 30-day data retention window. Backend endpoint POST /api/users/delete-account marks user in Firestore deletionQueue with 30-day scheduled deletion timestamp. POST /api/users/cancel-deletion endpoint allows cancellation by signing back in.
[x] 99. **REMOVED SUPPORT DEVELOPER AD FROM SETTINGS:** Removed SupportDeveloperAd component and its import from settings.tsx — the "watching one ad helps us..." text is gone from the page.
[x] 100. **AUTH CONTEXT DELETE ACCOUNT:** Added deleteAccount() to AuthContextValue type and implementation — calls backend to schedule deletion, then signs out from Firebase and Google gracefully.
[x] REPLIT MIGRATION VERIFIED: All npm packages installed, Start Backend (port 5000) running successfully, Start Frontend (Expo Metro port 8081) running successfully. Project fully operational in Replit environment.
[x] 101. **FIXED isInitializing MISSING FROM CONTEXT VALUE:** isInitializing was declared in MediaContextValue interface but omitted from the exported value object — any consumer received undefined instead of a boolean. Added isInitializing to the value object in MediaContext.tsx.
[x] 102. **FIXED VIDEO PLAYER MEMORY LEAK FOR IMAGES:** useVideoPlayer was creating a native video player for every item (including images), wasting native resources. Changed to pass null as the source for image items so no native video player is allocated.
[x] 103. **FIXED RACE CONDITION IN prepareStatusForViewing:** When the 1.5s timeout fired first, the copy could still complete in the background, producing an orphaned cache file. Fixed by tracking a timedOut flag — if the copy arrives after the timeout, the temp file is immediately deleted and the original URI is returned instead, preventing stale cache accumulation.
[x] 104. **FIXED VIDEO BLACK SCREEN BUG - THREE ROOT CAUSES:** (1) Player no longer initialized with raw content:// URI — always starts with null source, preventing decoder failure on Android. (2) player.replace() now waits 100ms before issuing play() so the decoder has time to settle and receive the first frame. (3) Added key={item.id} to VideoView so React correctly mounts a fresh native view per video item instead of reusing a stale one when swiping.
[x] 106. **FIXED VIDEO BLACK SCREEN / SOUND WITH NO VIDEO (3 ROOT CAUSES):** (1) Replaced setTimeout+play() with useEventListener statusChange — play() now fires ONLY when player reports 'readyToPlay', never on a timing guess. (2) replaceAsync now used on ALL platforms (not just iOS) so the UI thread is never blocked during video load. (3) Removed race-condition from prepareStatusForViewing — file is fully awaited before its URI is returned, so the player never receives a half-written file. isActiveRef keeps the event listener in sync with current active state to avoid stale closures.
[x] 105. **SUBSCRIPTION PAGE REDESIGNED AS REAL PAYMENT PAGE:** Complete visual overhaul — hero section with trust badges (Server Verified, Razorpay Secure, Google Account), "What you get" feature list, large prominent price display (₹30/₹199/₹499) with "Pay ₹X" buttons (never "Sign in first"), "Why subscribe?" section with icons, secure footer. Pay button auto-triggers Google sign-in if not signed in then proceeds directly to payment (pendingPlanRef pattern). Signed-in users shown as account row. Best Value plan has highlighted strip. No sign-in button clutter on pay buttons.
[x] REPLIT MIGRATION COMPLETE (2024): All npm packages installed successfully (tsx + expo binaries confirmed), Start Backend running on port 5000, Start Frontend (Expo Metro) running on port 8081. Both workflows verified RUNNING. Project fully operational in Replit environment.
[x] 107. **FIXED STORAGE LEAK:** cleanupCacheFiles() now runs automatically on app startup via a dedicated useEffect placed after the function is defined — cache files (view_* and share_*) older than 4 hours are deleted on every launch.
[x] 108. **FIXED GALLERY SAVING:** Uncommented MediaLibrary.createAssetAsync and createAlbumAsync in saveStatus — saved statuses now appear in the device Gallery / Google Photos under a "StatusVault" album, not just inside the app.
[x] 109. **FIXED AD FREQUENCY:** Changed image swipe interstitial from random 7–10 to a fixed 8 swipes for a predictable, consistent user experience.
[x] 110. **ZERO-LATENCY SWIPE (PRE-LOADING):** Added background pre-caching in viewer.tsx — when user is on item N, items N+1 and N+2 are silently prepared in cache. No more black screens from on-demand disk copies during fast swiping.
[x] 111. **INSTANT SHARING (CACHE REUSE):** shareStatus now reuses the existing view_ cache file if the item was already opened — no redundant disk copy. Falls back to a deduplicated share_ file (keyed by item ID, not timestamp) so sharing the same item never creates duplicates.
[x] 112. **SCROLL DEBOUNCE (60ms):** onScroll handler now uses a 60ms debounce — rapid flicks skip intermediate pages and only process the final settled position, preventing I/O clog that caused black screens.
[x] 113. **MEMORY OPTIMIZATION:** FlatList maxToRenderPerBatch reduced from 3→1, initialNumToRender 3→1, updateCellsBatchingPeriod 10→50ms — items are rendered one at a time to prevent simultaneous disk I/O competing for the disk.
[x] 114. **INITIALIZATION FLICKER FIX:** index.tsx now checks isInitializing from MediaContext — while the permission/storage check is running, a centered ActivityIndicator is shown instead of the "Setup Required" screen. Users who already granted access never see a false permission prompt again.
[x] 115. **EDUCATIONAL EMPTY STATE:** Empty state text changed from "No images/videos found" to "No image/video statuses yet — Open WhatsApp and view some statuses first, they will appear here automatically!" Turns a perceived failure into an actionable guide.
[x] 116. **SILENT GALLERY FAILURE FIX:** MediaLibrary.createAssetAsync errors now surface an Alert: "Gallery Access Denied — Status saved in the app, but could not be added to your Gallery. Allow media access in Settings." Users are never misled.
[x] 117. **INSTANT PRO MODE (SUBSCRIPTION CACHE):** useSubscriptionStatus now loads the last-known subscription status from AsyncStorage on mount. Paid users start the app already in Pro Mode with ads hidden — server check runs in the background without any flash of ads.
[x] 118. **FIRESTORE NOTICE SYSTEM:** Created hooks/useAppNotice.ts + components/AppNotice.tsx. Reads message/title/active/id from Firestore appConfig/notice document. Only shows if active=true and message is non-empty. Shows max 2 times (tracks per-notice via AsyncStorage). Has X close button. Styled with green accent border, gradient background, and megaphone icon. Added to home page between source selector and tab bar.
[x] 119. **SAVED TAB FLICKER FIX:** saved.tsx now checks isInitializing — shows a centered spinner while MediaContext loads, preventing the "Nothing saved yet" false empty state from flashing on startup.
[x] 120. **AUTO-REFRESH ON FOREGROUND:** AppState listener added to index.tsx — when user returns from WhatsApp (or any other app), refresh() is silently called so new statuses appear automatically without manual pull-to-refresh.
[x] 121. **PRELOAD STAGGER (200ms):** Pre-loading in viewer.tsx now loads item N+1 immediately and item N+2 after a 200ms delay. Two simultaneous disk copies no longer congest slow storage on budget Android phones.
[x] 122. **DELETE ACCOUNT SAFETY (GDPR):** AuthContext.tsx deleteAccount now requires a confirmed 200 OK from the server before signing out. If the server is unreachable, the user is kept signed in and shown a clear error. Account data can never be "orphaned" on the server while the user thinks it was deleted.
[x] 123. **FEEDBACK & CONTACT PAGE:** Created app/contact.tsx — full-screen Support page accessible from Settings > Support. Two form tabs: "Feedback" and "Report a Bug". Collects name (optional), email (optional), message. Writes to Firestore `feedback` collection with fields: type, message, name, contactEmail, userId, userEmail, priority (true for Pro subscribers), deviceModel, osVersion, status='open', createdAt. Pro subscribers see a gold crown banner and get priority:true — their reports surface first. Also shows direct contact info: email ahmedsameerbinan2@gmail.com (tap to compose mail) and phone +91 9567873283 (tap opens WhatsApp/phone). Registered as Stack.Screen in _layout.tsx (headerShown: false — has its own custom header with back button).
